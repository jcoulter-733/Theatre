...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_FILES["image_url"]["error"] == 0) {
    ...
    if (in_array($fileType, $allowTypes)) {
        ...
        if (move_uploaded_file($_FILES["image_url"]["tmp_name"], $targetFilePath)) {
            $addBlog = $conn->prepare("INSERT INTO shows (show_name, show_info, show_type, date_shown) VALUES(?, ?, ?, ?)");
            $addBlog->bind_param('ssss', $_POST['name'], $_POST['info'], $_POST['showType'], $_POST['date']);

            if ($addBlog->execute()) {
                $_SESSION['statusMsg'] = "The file " . $fileName . " has been uploaded and show added successfully.";
            } 
        } else {
